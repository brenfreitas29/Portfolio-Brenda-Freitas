...
export default function SideAtmosphere() {
  ...
  // Mouse spotlight now follows the pointer directly — no spring/lag.
  ...
  const spotlightBackground = useTransform(
    [pointerX, pointerY],
    ([x, y]) => `radial-gradient(560px circle at ${x}% ${y}%, rgba(224,151,255,.13), transparent 56%)`
  );
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[5] overflow-hidden" aria-hidden="true">
      ...
      <motion.div style={{ backgroundImage: spotlightBackground }} className="absolute inset-0 hidden will-change-[background-image] lg:block" />

      <motion.div
        style={{ x: reduceMotion ? 0 : auroraOneX, y: reduceMotion ? 0 : auroraOneY, rotate: reduceMotion ? 0 : backgroundRotate }}
        animate={reduceMotion ? undefined : { scale: [1, 1.12, .98, 1] }}
      ...
      <motion.div
        ...
        animate={reduceMotion ? undefined : { scale: [.95, 1.08, 1, .95], rotate: [0, 8, -5, 0] }}
      ...
      <div
        ...
        style={{
          backgroundImage: "linear-gradient(rgba(255,255,255,.42) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.42) 1px,transparent 1px)",
      ...
      <motion.div
        style={{ y: reduceMotion ? 0 : constellationY, x: reduceMotion ? 0 : constellationX, rotate: reduceMotion ? 0 : constellationRotate }}
        className="absolute -left-[2vw] top-[48vh] hidden h-[430px] w-[104vw] lg:block"
      >
        ...
        <svg className="absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 1200 430" preserveAspectRatio="none">
          <motion.path
            d="M90 225 C205 220 240 90 350 120 S490 300 590 235 S720 70 835 120 S1010 305 1110 225"
            fill="none"
            stroke="rgba(238,208,255,.34)"
            strokeWidth="1.2"
        ...
        {techNodes.map((node, index) => (
          <motion.div
            ...
            animate={reduceMotion ? undefined : { y: [0, -12, 5, 0] }}
            transition={{ duration: 6 + index * .8, repeat: Infinity, ease: "easeInOut", delay: node.delay }}
          >
            <motion.div
              animate={reduceMotion ? undefined : { rotate: 360 }}
              transition={{ duration: 20 + index * 3, repeat: Infinity, ease: "linear" }}
              className="relative flex items-center justify-center rounded-full border border-[#ecccf8]/30 bg-[#a875ff]/[.055] shadow-[0_0_55px_rgba(219,156,255,.20)]"
            ...
            >
              ...
              <motion.span
                animate={reduceMotion ? undefined : { rotate: -360 }}
                transition={{ duration: 20 + index * 3, repeat: Infinity, ease: "linear" }}
                className="text-[10px] font-semibold tracking-[.18em] text-white/80"
              >
                {node.label}
              </motion.span>
            </motion.div>
      ...
      <div className="hidden lg:block">
        ...
        <motion.div style={{ y: railY }} className="absolute left-10 top-[30vh] flex flex-col gap-9 xl:left-12">
          {["01", "02", "03"].map((n) => (
            <div key={n} className="flex items-center gap-3"><span className="h-px w-5 bg-white/18" /><span className="text-[9px] font-medium tracking-[.22em] text-white/28">{n}</span></div>
          ))}
        </motion.div>
        ...
        {particles.map((particle, index) => (
          <motion.span
            ...
            animate={reduceMotion ? undefined : { y: [0, -18, 7, 0], opacity: [.18, .75, .32, .18], scale: [.9, 1.3, 1, .9] }}
            transition={{ duration: 7 + index * .65, repeat: Infinity, ease: "easeInOut", delay: particle.delay }}
          />
        ))}
