      ...
      <div className="relative overflow-hidden rounded-[1.8rem] border border-white/[.09] bg-[#170f31] shadow-[0_20px_60px_rgba(5,1,24,.22)]">
        ...
        <div className={`relative z-10 grid gap-8 p-6 sm:p-8 lg:grid-cols-[.44fr_1.06fr] lg:items-center lg:p-10 xl:p-12 ${flip ? "lg:grid-cols-[1.06fr_.44fr]" : ""}`}>
          <div className={flip ? "lg:order-2" : ""}>
            <div className="flex items-center gap-3 text-sm uppercase tracking-[.12em] text-[#e7dffa]">
              <span>Project {project.number}</span><span className="h-px w-8 bg-white/15"/><span>03</span>
            </div>
            <p className="mt-5 text-sm uppercase tracking-[.12em] text-[#efb7df]">{project.category}</p>
            <h3 className="mt-4 text-[clamp(3rem,5vw,5.8rem)] font-medium leading-[.88] tracking-[-.065em]">{project.title}</h3>
            <p className="mt-6 max-w-lg text-sm leading-7 text-[#e7dffa] md:text-base">{project.description}</p>

            <div className="mt-7 flex max-w-lg flex-wrap gap-2">
              {project.tags.map(tag => <span key={tag} className="rounded-full border border-white/[.09] bg-white/[.025] px-3 py-1.5 text-xs tracking-[.04em] text-[#e7dffa]">{tag}</span>)}
            </div>

            <div className="mt-8 flex flex-wrap gap-3">
              <Magnetic strength={.14}><Link href={project.href} className="inline-flex rounded-full bg-white px-5 py-3 text-sm font-semibold text-[#24194f] transition-transform duration-200 hover:-translate-y-0.5">Case study ↗</Link></Magnetic>
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full border border-white/14 px-5 py-3 text-sm text-white transition-colors hover:border-white/25 hover:text-white">Open product ↗</a>
              <a href={project.codeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-2 py-3 text-sm text-[#e7dffa] transition-colors hover:text-[#efb7df]">Code ↗</a>
            </div>
          </div>
          ...
          <div className={`relative ${flip ? "lg:order-1" : ""}`}>
            <Link href={project.href} className="group relative block overflow-hidden rounded-[1.45rem] border border-white/[.11] bg-[#0f0a23] p-3 shadow-[0_24px_70px_rgba(4,1,19,.34)] md:p-4" aria-label={`View ${project.title}`}>
              <div className="flex h-9 items-center gap-2 border-b border-white/[.07] px-1">
                ...
                <span className="ml-auto text-xs tracking-[.08em] text-[#e7dffa]">Selected work / {project.number}</span>
              </div>
