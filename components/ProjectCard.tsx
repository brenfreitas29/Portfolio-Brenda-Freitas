...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  const sceneY = useTransform(scrollYProgress,[0,.5,1],reduceMotion?[0,0,0]:[36,0,-36]);
  const sceneScale = useTransform(scrollYProgress,[0,.5,1],reduceMotion?[1,1,1]:[.98,1,.985]);
  const previewY = useTransform(scrollYProgress,[0,.5,1],reduceMotion?[0,0,0]:[24,0,-26]);
  const previewRotate = useTransform(scrollYProgress,[0,.5,1],reduceMotion?[0,0,0]:[flip?-1.6:1.6,0,flip?.6:-.6]);
  const giantX = useTransform(scrollYProgress,[0,1],reduceMotion?[0,0]:[flip?36:-36,flip?-42:42]);
  const isRemoteImage = project.image.startsWith("http");

  return (
    <motion.article ref={ref} style={{ y:sceneY }} className="relative min-h-[94vh] lg:min-h-[100vh]">
      <div className="sticky top-4 flex min-h-[calc(100vh-2rem)] items-center overflow-hidden rounded-[2rem] border border-white/10 bg-[#170f31]/95 shadow-[0_24px_70px_rgba(5,1,24,.26)]">
        <motion.div style={{ x:giantX }} className="pointer-events-none absolute inset-x-0 top-[8%] whitespace-nowrap text-center text-[clamp(6rem,18vw,18rem)] font-semibold leading-none tracking-[-.1em] text-white/[.022]">{project.title.toUpperCase()}</motion.div>
        <div className={`relative z-10 grid w-full items-center gap-8 px-6 py-8 md:px-9 lg:grid-cols-[.42fr_1.18fr] lg:px-10 xl:px-14 ${flip ? "lg:grid-cols-[1.18fr_.42fr]" : ""}`}>
          ...
          <motion.div style={{ y:previewY, rotate:previewRotate, scale:sceneScale }} className={`relative ${flip ? "lg:order-1" : ""}`}>
            <Link href={project.href} className="relative block overflow-hidden rounded-[1.6rem] border border-white/14 bg-[#0f0a23] p-3 shadow-[0_34px_90px_rgba(4,1,19,.38)] md:p-4" aria-label={`View ${project.title}`}>
              <div className="flex h-10 items-center gap-2 border-b border-white/8 px-2"><span className="h-2.5 w-2.5 rounded-full bg-[#f2abc9]/75"/><span className="h-2.5 w-2.5 rounded-full bg-[#e6cb8f]/70"/><span className="h-2.5 w-2.5 rounded-full bg-[#9bd5a5]/70"/><span className="ml-auto text-[8px] uppercase tracking-[.2em] text-white/22">Project preview</span></div>
              <div className="relative aspect-[16/10] overflow-hidden rounded-b-[1.05rem] bg-white">
                {isRemoteImage ? <img src={project.image} alt={`${project.title} preview`} className="h-full w-full object-cover" loading="lazy"/> : <Image src={project.image} alt={`${project.title} preview`} fill className="object-cover" sizes="(max-width:1024px) 100vw,58vw" quality={82}/>}              
              </div>
            </Link>
            <div className={`absolute ${flip?"-left-4":"-right-4"} -top-5 hidden rounded-[1rem] border border-white/12 bg-[#26194e]/92 px-4 py-3 text-[8px] uppercase tracking-[.23em] text-[#efbddf]/62 shadow-[0_12px_30px_rgba(5,2,24,.24)] lg:block`}>Selected / {project.number}</div>
          </motion.div>
        </div>
      </div>
    </motion.article>
